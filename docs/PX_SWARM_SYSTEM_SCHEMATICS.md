...
- The repository README still refers to Isaac Sim and Pegasus as the target simulation stack, while the included launcher scripts automate Gazebo + PX4 SITL. The control layer itself only depends on PX4/MAVSDK and the swarm UDP bus, so it can sit above either simulator stack as long as PX4 exposes MAVLink UDP.
